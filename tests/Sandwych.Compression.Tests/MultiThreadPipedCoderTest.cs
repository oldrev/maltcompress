...
public class MultiThreadPipedCoderTest : AbstractCompressionTest
{
    ...
    public void SingleThreadShouldBeOk()
    {
        ...
        using (var pipedCoder = new MultiThreadPipedCoder<SynchronizedStreamConnector2>(new CopyCoder(1024)))
    ...
    public void Test4ThreadsCoder()
    {
        ...
        using (var pipedCoder = new MultiThreadPipedCoder<SynchronizedStreamConnector2>(new CopyCoder(1024), new XorCoder(xorKey, 512), new CopyCoder(2048), new XorCoder(xorKey, 4096)))
        using (var inStream = new MemoryStream(sourceData, false))
        using (var outStream = new MemoryStream())
        {
