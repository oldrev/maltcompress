...
public class AsyncStreamConnector : IAsyncStreamConnector, IAsyncDisposable
{
	...
	public async ValueTask Produce(ReadOnlyMemory<byte> buffer, CancellationToken cancel)
	{
		if (_disposed)
		{
			throw new ObjectDisposedException(nameof(AsyncStreamConnector));
	...
	public ValueTask<int> Consume(Memory<byte> buffer, CancellationToken cancel)
	{
		if (_disposed)
		{
			throw new ObjectDisposedException(nameof(AsyncStreamConnector));
	...
	public ValueTask ProducerDisposed()
	{
		if (_disposed)
		{
			throw new ObjectDisposedException(nameof(AsyncStreamConnector));
	...
	public ValueTask ConsumerDisposed()
	{
		return ValueTask.CompletedTask;
	}
